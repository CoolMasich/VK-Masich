...
if len(humans) > 0:
    print(f"[pink]Вконтакте вернул {len(humans)} друзей, которые не посещали свою "
          f"страницу больше 1 дня[/pink]\n[cyan]Начинаю удалять...[/cyan]")
    ...
    while i < len(humans):
        ...
        if "response" in data:
            print(f"[pink]id{humans[i]}[/pink]: Удалил")
        elif "error" in data:
            ...
            print(f"[pink]id{humans[i]}[/pink]: Не смог удалить")
        else:
    ...
    print("[green]У вас нет друзей, которые не заходили больше 1 дня[/green]")
    time.sleep(1)
    os.system('python start.py')

...
if len(humans) > 0:
    print(f"[green]Удалось получить {len(humans)} пользователей[/green]")
    ...
    while i < len(humans):
        ...
        if "response" in data:
            if data['response'] == 1:
                print(f"[pink]id{humans[i]}[/pink]: Добавил")
            elif data['response'] == 2:
                print(f"[pink]id{humans[i]}[/pink]: Уже в друзьях")
            else:
                print(f"[pink]id{humans[i]}[/pink]: Добавил")
        elif "error" in data:
            if data['error']["error_code"] == 14:
                ...
                if "response" in data_captcha:
                    print("[green]Верно[/green]\n"
                          f"[pink]id{humans[i]}[/pink]: Добавил")
